...
\hypersetup{
    ...
    pdftitle={llamatelemetry v2.2.0 Portfolio - Waqas Muhammad},
...
\fancyhead[L]{\textcolor{primaryblue}{\small llamatelemetry v2.2.0 Portfolio}}
...
\begin{titlepage}
    ...
    {\Huge\bfseries\textcolor{primaryblue}{llamatelemetry v2.2.0}\par}
    \vspace{0.3cm}
...
llamatelemetry v2.2.0 introduces a novel \textbf{split-GPU architecture pattern} optimized for Kaggle's dual T4 environment. This design enables simultaneous LLM inference and GPU-accelerated visualization without resource contention.
...
\end{tikzpicture}
\caption{llamatelemetry v2.2.0 Split-GPU Architecture on Kaggle Dual T4}
\end{figure}
...
llamatelemetry v2.2.0 includes 11 comprehensive Kaggle notebooks that progressively build expertise from beginner to advanced topics. Notebooks 01-10 establish foundational skills, while Notebook 11 (detailed in Section 4) demonstrates the flagship visualization capabilities.
...
\vspace{0.3cm}
\normalsize
The culminating demonstration of llamatelemetry v2.2.0's capabilities, showcasing a groundbreaking approach to visualizing the internal architecture of GGUF quantized models through 8 interactive Graphistry dashboards.
...
\vspace{0.2cm}
\textbf{Primary:} \texttt{pip install git+https://github.com/llamatelemetry/llamatelemetry.git@v2.2.0}\\
...
\hline
\end{tabularx}
\caption{llamatelemetry v2.2.0 API Design Principles}
\end{table}
...
\hline
\end{tabularx}
\caption{llamatelemetry v2.2.0: Cross-Domain Impact}
\end{table}
...
\Large\textbf{llamatelemetry v2.2.0 - Official Links}
...
!pip install -q --no-cache-dir --force-reinstall \
    git+https://github.com/llamatelemetry/llamatelemetry.git@v2.2.0
...
\subsection{Why llamatelemetry v2.2.0 Matters}
...
\begin{tcolorbox}[colback=white,colframe=accentblue,width=0.9\textwidth,arc=2mm]
\centering
\textit{llamatelemetry v2.2.0 represents the intersection of GPU systems engineering, ML infrastructure, and AI explainability—demonstrating both technical depth and practical impact on free, accessible compute infrastructure.}
